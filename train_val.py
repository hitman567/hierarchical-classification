...
alpha = K.variable(value=0.7, dtype="float32", name="alpha")
beta = K.variable(value=0.3, dtype="float32", name="beta")
...
def generator(batch_size, target_size, folder):
    ...
    while True:
        ...
        yield input_images, [h1_groundtruths, h2_groundtruths]
...
def BCNN_model(input_shape, num_h1, num_h2):
    ...
    model = Model(inputs=img_input, outputs=[c_1_pred, c_2_pred], name='hierarchical_classifier')

    return(model)

hierarchical_model = BCNN_model((target_size[0], target_size[1], 3), num_h1, num_h2)
